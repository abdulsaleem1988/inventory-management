<template>
  <div class="reports">
    ...
    <div v-else>
      ...
      <div class="card">
        <div class="card-header">
          <h3 class="card-title">{{ t("reports.monthly.trendTitle") }}</h3>
        </div>
        <div class="chart-container">
          <div class="bar-chart">
            ...
            >
              ...
              <div class="bar-label">{{ getLocalizedMonth(month.month) }}</div>
            </div>
          </div>
        </div>
      </div>
          ...
          <table class="reports-table">
            ...
            <tbody>
              <tr v-for="(month, index) in monthlyData" :key="month.month">
                <td>
                  <strong>{{ getLocalizedMonth(month.month) }}</strong>
                </td>
...
const calculateSummaryStats = () => {
  ...
  avgMonthlyRevenue.value =
    monthlyData.value.length > 0 ? total / monthlyData.value.length : 0;
  ...
  const best = quarterlyData.value.reduce((max, q) =>
    q.total_revenue > (max?.total_revenue || 0) ? q : max,
  );
...
const getLocalizedMonth = (monthStr) => {
...
watch(
  [selectedLocation, selectedCategory, selectedStatus, selectedPeriod],
  () => {
    loadData();
  },
);
