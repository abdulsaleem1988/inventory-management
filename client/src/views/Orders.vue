<template>
  <div class="orders">
    ...
    <div v-else>
      ...
      <div v-if="restockOrders.length > 0" class="card" style="margin-top: 24px;">
        <div class="card-header">
        ...
        <div class="table-container">
          <table class="orders-table">
            <thead>
              <tr>
                <th class="col-order-number">{{ t('orders.table.orderNumber') }}</th>
                <th class="col-customer">Warehouse</th>
                <th class="col-items">{{ t('orders.table.items') }}</th>
                <th class="col-value">{{ t('orders.table.totalValue') }}</th>
                <th class="col-date">{{ t('orders.table.orderDate') }}</th>
            ...
            <tbody>
              <tr v-for="order in restockOrders" :key="order.id">
                ...
                <td class="col-date">{{ formatDate(order.order_date) }}</td>
                <td class="col-date">{{ formatDate(order.expected_delivery) }}</td>
                <td class="col-date"><span class="badge info">{{ leadTimeDays(order) }} days</span></td>
              </tr>
            </tbody>
          </table>
        </div>
...
export default {
  ...
  setup() {
    ...
    const restockOrders = computed(() => {
      return orders.value.filter(o => o.order_number?.startsWith('RST-'))
    })

    const leadTimeDays = (order) => {
      const placed = new Date(order.order_date)
      const delivery = new Date(order.expected_delivery)
      return Math.round((delivery - placed) / (1000 * 60 * 60 * 24))
    }
